...
class Fieldmanager_Context_QuickEdit extends Fieldmanager_Context_Storable {
	...
	public function __construct( $title, $post_types, $column_display_callback, $column_title = '', $fm = null ) {
		...
		$post_type = ! isset( $_GET['post_type'] ) ? 'post' : sanitize_text_field( wp_unslash( $_GET['post_type'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification, input var okay
	...
	public function render_ajax_form() {
		if ( ! isset( $_GET['action'], $_GET['post_id'], $_GET['column_name'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification, input var okay
			return;
		}

		if ( 'fm_quickedit_render' != $_GET['action'] ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification, input var okay
			return;
		}

		$column_name = sanitize_text_field( wp_unslash( $_GET['column_name'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification, input var okay
